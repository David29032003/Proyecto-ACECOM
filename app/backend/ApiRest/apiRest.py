...
def recomendaciones_colaborativo(id_usuario, cantidad_recomendaciones=5):
    ...
    peliculas_recomendadas = [diccionario_pelicula[pelicula[0]] for pelicula in peliculas_ordenadas[:cantidad_recomendaciones] if pelicula[0] in diccionario_pelicula]
...
def buscar_peliculas_con_embedding(texto):
    embedding_texto = modelo_buscador.encode([texto])
    embeddings_peliculas = modelo_buscador.encode(dataset1['Overview'].tolist())
    similitudes = cosine_similarity(embedding_texto, embeddings_peliculas)
    ...
    for id, fila in dataset1.iterrows():
        ...
        generos_pelicula = [genero.strip().lower() for genero in fila['Genre'].split(',')]
        
        if any(genre in generos_texto for genre in generos_pelicula):
            similitudes[0][id] *= factor
...
def contenido():
    ...
    recomendaciones = recomendaciones_contenido(titulo)
...
def colaborativo():
    ...
    recomendaciones = recomendaciones_colaborativo(id_usuario)
